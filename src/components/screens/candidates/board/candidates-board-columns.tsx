  ...
  return (
    <motion.div
      ...
      animate={{
        ...
        scale: isOver ? 1.01 : 1,
        boxShadow: isOver
          ? "0 22px 50px -30px rgba(14, 165, 233, 0.45)"
          : "0 0 0 rgba(0,0,0,0)",
      }}
...
function CandidateCard({
  ...
  return (
    ...
    >
      <div className="mb-2">
        <Badge
          className={`${getJobColor(candidate.jobId)} border text-xs font-medium`}
        >
          {candidate.jobTitle}
        </Badge>
      </div>
      ...
      <div className="flex items-start gap-3">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-info/20 bg-info/10">
        ...
        <div className="min-w-0 flex-1 space-y-0.5">
          ...
          <p className="truncate text-xs text-muted-foreground">
            ...
            {candidate.availability || "Open to discuss"}
          </p>
        </div>
      </div>
      ...
      <div className="mt-3 flex items-center justify-between gap-3">
        <Badge
          className={`${getEngagementColor(candidate.engagement)} border-0 text-xs`}
        >
